...
//homepage
app.get('/', (req,res)=>{                
	db.collection('sportsGoods').find().toArray( (err,result)=>{     
		if(err) return console.log(err)     
...
app.post('/AddData', (req,res)=>{			
	db.collection('sportsGoods').save(req.body, (err,result)=>{			
		if(err) return console.log(err)
	res.redirect('/')                        
	})
})
...
app.post('/update', (req,res)=>{			
		db.collection('sportsGoods').find().toArray((err,result)=>{	   			
			...
			db.collection('sportsGoods').findOneAndUpdate({pid:req.body.id},{
				$set:{stock: parseInt(s) +parseInt(req.body.stock)}} , {sort:{_id:-1}},
				(err,result) =>{
					if(err)
						return console.log(err)
					console.log(req.body.id+'stock has been updated')
					res.redirect('/')
				})
			})	
		})
...
app.post('/delete',(req,res)=>{
	db.collection('sportsGoods').findOneAndDelete({pid:req.body.id}, (err,result)=>{    
		if(err)
			return console.log(err)
		res.redirect('/')
	})
